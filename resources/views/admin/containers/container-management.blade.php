    ...
    <x-slot name="header">
        <div class="flex items-start justify-between gap-4">
            <div class="flex flex-col gap-2">
                ...
                <h2 class="text-3xl font-black tracking-tight text-slate-900">Gestió de Contenidors</h2>
            </div>
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <section class="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 px-6 py-5">
                    ...
                    <h3 class="mt-2 text-2xl font-black tracking-tight text-slate-900">Contenidors registrats</h3>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left font-black uppercase tracking-[0.14em] text-slate-600">Entrada</th>
                                <th class="px-6 py-4 text-left font-black uppercase tracking-[0.14em] text-slate-600">Sortida</th>
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @forelse ($containers as $container)
                                <tr class="hover:bg-slate-50">
                                    <td class="px-6 py-4 font-semibold text-slate-900">{{ $container->id }}</td>
                                    <td class="px-6 py-4 text-slate-700">{{ $container->name }}</td>
                                    <td class="px-6 py-4 text-slate-700">{{ $container->input ? 'Sí' : 'No' }}</td>
                                    <td class="px-6 py-4 text-slate-700">{{ $container->output ? 'Sí' : 'No' }}</td>
                                    <td class="px-6 py-4">
                                        <div class="flex justify-end gap-2">
                                            <a href="{{ route('containers.edit', $container) }}" class="inline-flex items-center rounded-xl bg-sky-100 px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-sky-700 transition hover:bg-sky-200">
                                                Editar
                                            </a>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-10 text-center text-slate-500">Encara no hi ha registres disponibles.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
