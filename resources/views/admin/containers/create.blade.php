<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2">
            ...
            <h2 class="text-3xl font-black tracking-tight text-slate-900">Nou Contenidor</h2>
        </div>
    </x-slot>
    ...
    <div class="py-10">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <section class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-7">
                <form method="POST" action="{{ route('containers.store') }}" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <input id="weight" name="weight" type="number" min="0" value="{{ old('weight') }}" class="mt-3 w-full rounded-2xl border border-slate-300 px-4 py-3 text-slate-900 focus:border-emerald-600 focus:outline-none focus:ring-4 focus:ring-emerald-100">
                        @error('weight')
                            <p class="mt-2 text-sm font-semibold text-rose-600">{{ $message }}</p>
                        @enderror
                    </div>
