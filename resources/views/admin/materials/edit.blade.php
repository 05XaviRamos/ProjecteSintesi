<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2">
            ...
            <h2 class="text-3xl font-black tracking-tight text-slate-900">Editar Material</h2>
        </div>
    </x-slot>
            ...
            <section class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-7">
                <form method="POST" action="{{ route('materials.update', $material) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input id="name" name="name" type="text" value="{{ old('name', $material->name) }}" class="mt-3 w-full rounded-2xl border border-slate-300 px-4 py-3 text-slate-900 focus:border-emerald-600 focus:outline-none focus:ring-4 focus:ring-emerald-100">
                        @error('name')
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <label class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 font-semibold text-slate-700">
                            <input type="checkbox" name="input" value="1" {{ old('input', $material->input) ? 'checked' : '' }} class="h-5 w-5 rounded border-slate-300 text-emerald-700 focus:ring-emerald-500">
                            Entrada
                        </label>

                        <label class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 font-semibold text-slate-700">
                            <input type="checkbox" name="output" value="1" {{ old('output', $material->output) ? 'checked' : '' }} class="h-5 w-5 rounded border-slate-300 text-emerald-700 focus:ring-emerald-500">
                            Sortida
                        </label>
                    </div>
                    ...
                    <div class="rounded-[1.6rem] border border-slate-200 bg-white p-5 shadow-sm">
                        ...
                        @php($selectedZones = old('zones', $material->zones->pluck('id')->all()))
