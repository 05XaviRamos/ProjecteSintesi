    ...
    <x-slot name="header">
        <div class="flex items-start justify-between gap-4">
            <div class="flex flex-col gap-2">
                ...
                <h2 class="text-3xl font-black tracking-tight text-slate-900">Gestió de Materials</h2>
            </div>
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <section class="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 px-6 py-5">
                    ...
                    <h3 class="mt-2 text-2xl font-black tracking-tight text-slate-900">Materials registrats</h3>
                </div>
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @forelse ($materials as $material)
                                <tr class="hover:bg-slate-50">
                                    <td class="px-6 py-4 font-semibold text-slate-900">{{ $material->id }}</td>
                                    <td class="px-6 py-4 text-slate-700">{{ $material->name }}</td>
                                    <td class="px-6 py-4 text-slate-700">{{ $material->input ? 'Sí' : 'No' }}</td>
                                    <td class="px-6 py-4 text-slate-700">{{ $material->output ? 'Sí' : 'No' }}</td>
                                    <td class="px-6 py-4">
                                        <div class="flex justify-end gap-2">
                                            <a href="{{ route('materials.edit', $material) }}" class="inline-flex items-center rounded-xl bg-sky-100 px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-sky-700 transition hover:bg-sky-200">
                                                Editar
                                            </a>
