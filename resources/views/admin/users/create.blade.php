<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2">
            ...
            <h2 class="text-3xl font-black tracking-tight text-slate-900">Nou Usuari</h2>
        </div>
    </x-slot>
            ...
            <section class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-7">
                <form method="POST" action="{{ route('users.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input id="email" name="email" type="email" value="{{ old('email') }}" class="mt-3 w-full rounded-2xl border border-slate-300 px-4 py-3 text-slate-900 focus:border-emerald-600 focus:outline-none focus:ring-4 focus:ring-emerald-100">
                        @error('email')
                            <p class="mt-2 text-sm font-semibold text-rose-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                        <div>
                            <label for="password" class="text-sm font-black uppercase tracking-[0.18em] text-slate-800">Contrasenya</label>
                        ...
                        <div>
                            <label for="password_confirmation" class="text-sm font-black uppercase tracking-[0.18em] text-slate-800">Confirmar contrasenya</label>
